                ...
                cy_en_syspm_callback_mode_t mode)
{
    cy_en_syspm_status_t retVal = CY_SYSPM_SUCCESS;
        ...
        (void)callbackParams;
        (void)mode;
    #endif /* CY_CAPSENSE_PLATFORM_BLOCK_FOURTH_GEN || CY_CAPSENSE_PLATFORM_BLOCK_FIFTH_GEN */
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    context->ptrWdContext[widgetId].cdacCompDivider = 
        (uint16_t)ptrBuffer[CY_CAPSENSE_CDAC_CCOMP_DIV_OFFSET] + (uint16_t)((uint16_t)ptrBuffer[CY_CAPSENSE_CDAC_CCOMP_DIV_OFFSET + 1u] << 8u);
    for (i = 0u; i < context->ptrWdConfig[widgetId].numSns; i++)
    {

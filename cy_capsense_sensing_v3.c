...
****************************************************************************//**
*
* Initiates the non-blocking scan of all slots. The scan finishing can be
* checked by using the Cy_CapSense_IsBusy() function.
*
* \param context
...
****************************************************************************//**
*
* Initiates the non-blocking scan of specified slots. The scan finishing can be
* checked by using the Cy_CapSense_IsBusy() function.
*
* \param startSlotId
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    {
        if (context->ptrCommonConfig->numSlots > lastSlot)
        {
            ...
            else
            {
                ...
                /* Initiates the frame start for each channel */
                for (curChIndex = 0u; curChIndex < context->ptrCommonConfig->numChannels; curChIndex++)
                {
                    Cy_MSC_WriteReg(context->ptrCommonConfig->ptrMscChConfig[curChIndex].ptrMscBase,
                ...
                for (curChIndex = 0u; curChIndex < context->ptrCommonConfig->numChannels; curChIndex++)
                {
                    ...
                    curSlotIndex = startSlotId + curChIndex * context->ptrCommonConfig->numSlots;
                    slotValue = context->ptrScanSlots[curSlotIndex].wdId;
                    if(CY_CAPSENSE_SLOT_EMPTY == slotValue)
                    {
                        /* CY_ID#XXXX */
                        scanSlotIndexValid = curSlotIndex;
                        if(context->ptrCommonConfig->numChannels > 1u)
                        {
                            if(curSlotIndex < context->ptrCommonConfig->numSlots)
                            {
                    ...
                    {
                        scanSlotIndexValid = (context->ptrScanSlots[curSlotIndex].snsId * context->ptrCommonConfig->numSlots) +
                                    (curSlotIndex % context->ptrCommonConfig->numSlots);
                    }
                    else
                    {
                        scanSlotIndexValid = curSlotIndex;
                    ...
                    else
                    {
                        ...
                        context->ptrSensorFrameContext[(lastSlot + 1u + curChIndex * context->ptrCommonConfig->numSlots) *
                                                       context->ptrInternalContext->snsConfigSize - 1u] |= MSC_SNS_CTL_LAST_Msk;
                        ptrSensorFrame = &context->ptrSensorFrameContext[(startSlotId + curChIndex * context->ptrCommonConfig->numSlots) * context->ptrInternalContext->snsConfigSize];
                    }

                    if ((NULL != context->ptrCommonContext->ptrSSCallback) &&
                        (curChIndex == (context->ptrCommonConfig->numChannels - 1u)))
                    {
                        context->ptrCommonContext->ptrSSCallback((cy_stc_active_scan_sns_t *)&context->ptrActiveScanSns[0u]);
                    }

                    if (CY_CAPSENSE_SCAN_MODE_INT_DRIVEN == context->ptrCommonConfig->scanningMode)
                    {
                        Cy_MSC_ConfigureScan(context->ptrCommonConfig->ptrMscChConfig[curChIndex].ptrMscBase,
...
****************************************************************************//**
*
* Initiates the non-blocking scan for all widgets/sensors. The scan finishing can be
* checked by using the Cy_CapSense_IsBusy() function.
*
* \param context
* The pointer to the CapSense context structure \ref cy_stc_capsense_context_t.
...
****************************************************************************//**
*
* Initiates the scanning of all sensors in the widget. The scanning is
* initiated only if no scan is in progress. The scan finishing can be
* checked by using the Cy_CapSense_IsBusy() function.
*
* \param widgetId
...
****************************************************************************//**
*
* Initiates the scanning of the selected sensor in the widget. The scanning is
* initiated only if no scan is in progress. The scan finishing can be
* checked by using the Cy_CapSense_IsBusy() function.
*
* \param widgetId
...
****************************************************************************//**
*
* Calibrates CapDACs for all slots to the specified target.
*
* \param context
* The pointer to the CapSense context structure \ref cy_stc_capsense_context_t.
*
...
****************************************************************************//**
*
* Calibrates CapDACs for a selected widget.
*
* \param widgetId
* Specifies the ID number of the widget. A macro for the widget ID can be found
* in the cycfg_capsense.h file defined as CY_CAPSENSE_<WIDGET_NAME>_WDGT_ID.
...
cy_capsense_status_t Cy_CapSense_SsInitialize(cy_stc_capsense_context_t * context)
{
    ...
    uint32_t i;
    uint32_t maxRawTmp;
    uint32_t maxRawTmpCol;
    uint32_t maxRawTmpRow;
    ...
    for (curChIndex = 0u; curChIndex < context->ptrCommonConfig->numChannels; curChIndex++)
    {
        ...
        if (CY_CAPSENSE_CTRLMUX_SENSOR_CONNECTION_METHOD == ptrCommonCfg->sensorConnection)
        {
            Cy_CapSense_GenerateAllSensorConfig(curChIndex,
                    &context->ptrSensorFrameContext[(context->ptrCommonConfig->numSlots * curChIndex) *
        ...
        {
            Cy_MSC_WriteReg(ptrCommonCfg->ptrMscChConfig[curChIndex].ptrMscBase, CY_MSC_REG_OFFSET_INTR_MASK,
    ...
    for (i = 0u; i < context->ptrCommonConfig->numWd; i++)
    {
        ...
        {
            maxRawTmpCol = ptrWdCfg->ptrWdContext->cicRate * ptrWdCfg->ptrWdContext->cicRate;
            maxRawTmpRow = maxRawTmpCol;
        }
        else
        {
            maxRawTmp = (context->ptrCommonConfig->numEpiCycles * ptrWdCfg->numChopCycles - 1u);

            maxRawTmpCol  = maxRawTmp;
            maxRawTmpCol += (ptrWdCfg->ptrWdContext->numSubConversions * ptrWdCfg->numChopCycles * ptrWdCfg->ptrWdContext->snsClk);
            ...
            {
                maxRawTmpRow  = maxRawTmp;
                maxRawTmpRow += (ptrWdCfg->ptrWdContext->numSubConversions * ptrWdCfg->numChopCycles * ptrWdCfg->ptrWdContext->rowSnsClk);
            }
            else
            {
                maxRawTmpRow = maxRawTmpCol;
        ...
        ptrWdCfg->ptrWdContext->maxRawCount = (maxRawTmpCol > MSC_STATUS1_RAW_COUNT_Msk) ?
                                              ((uint16_t)MSC_STATUS1_RAW_COUNT_Msk) : maxRawTmpCol;

        ptrWdCfg->ptrWdContext->sigPFC = (maxRawTmpRow > MSC_STATUS1_RAW_COUNT_Msk) ?
                                         ((uint16_t)MSC_STATUS1_RAW_COUNT_Msk) : maxRawTmpRow;
                ...
                const cy_stc_capsense_context_t * context)
{
    ...
    else
    {
        ...
        if (0u != eltdMptxOrder)
        {
            pattern = (pattern << eltdMptxOrder) | (pattern >> (CY_CAPSENSE_MPTX_MAX_ORDER - eltdMptxOrder));
        }
                ...
                const cy_stc_capsense_context_t * context)
{
    ...
    if (chIndex == context->ptrActiveScanSns[chIndex].ptrRxConfig->chId)
    {
...
void Cy_CapSense_ScanISR(void * capsenseContext)
{
    ...
    uint32_t curChIndex = cxt->ptrCommonConfig->numChannels;
    ...
    for (i = 0u; i < cxt->ptrCommonConfig->numChannels; i++)
    {
    ...
    if (curChIndex < cxt->ptrCommonConfig->numChannels)
    {
        /* Clear all pending interrupts of the MSC HW block */
        Cy_MSC_WriteReg(ptrCommonCfg->ptrMscChConfig[curChIndex].ptrMscBase, CY_MSC_REG_OFFSET_INTR,
                        CY_CAPSENSE_MSC_INTR_ALL_MSK);
        ...
        if (CY_CAPSENSE_SCAN_MODE_DMA_DRIVEN != ptrCommonCfg->scanningMode)
        {
            slotStcIndex = curChIndex * ptrCommonCfg->numSlots + ptrIntrCxt->currentSlotIndex;
            ...
            if (ptrIntrCxt->currentSlotIndex != ptrIntrCxt->endSlotIndex)
            {
                ...
                if (1 == cxt->ptrCommonConfig->numChannels)
                {
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    if (CY_CAPSENSE_CTRLMUX_SENSOR_CONNECTION_METHOD == ptrCommonCfg->sensorConnection)
    {
        ...
        if(CY_CAPSENSE_SCAN_MODE_DMA_DRIVEN == context->ptrCommonConfig->scanningMode)
        {
            curSlotIndex = ptrIntrCxt->slotIndex;
            slotStcIndex = chIndex * ptrCommonCfg->numSlots + curSlotIndex;
            slotPtr = &context->ptrScanSlots[slotStcIndex];
            for(; curSlotIndex <= ptrIntrCxt->endSlotIndex; curSlotIndex++)
            {
                if((slotPtr->wdId != CY_CAPSENSE_SLOT_SHIELD_ONLY) &&
                   (slotPtr->wdId != CY_CAPSENSE_SLOT_TX_ONLY) &&
                   (slotPtr->wdId != CY_CAPSENSE_SLOT_EMPTY))
                {
                    ptrWdCfg = &context->ptrWdConfig[slotPtr->wdId];
                    if(CY_CAPSENSE_CSX_GROUP == ptrWdCfg->senseGroup)
                    {
                        maxCount = context->ptrWdContext[slotPtr->wdId].maxRawCount;
                        ptrSnsCxt = &ptrWdCfg->ptrSnsContext[slotPtr->snsId];
    ...
    if (0u == context->ptrCommonContext->status)
    {
        ...
        {
            context->ptrCommonContext->ptrEOSCallback((cy_stc_active_scan_sns_t *)&context->ptrActiveScanSns[0u]);
        }
    }
}
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    uint32_t currSlotPos =  mscChIndex * context->ptrCommonConfig->numSlots;
        ...
        if (CY_CAPSENSE_STATUS_SUCCESS == calibStatus)
        {
                    ...
                    (ptrCommCfg->csxCdacAutocalEn == CY_CAPSENSE_ENABLE)))
            {
                ...
                for(snsIndex = 0u; snsIndex < ptrWdCfg->numSns; snsIndex++)
                {
                    ...
                    {
                        tmpRawCount = context->ptrWdContext[wdId].sigPFC;
                    }
                    else
                    {
                        tmpRawCount = context->ptrWdContext[wdId].maxRawCount;
                    }

...
* Constants
*******************************************************************************/
const cy_stc_msc_base_config_t cy_capsense_smTemplates[] = CY_CAPSENSE_SENSING_METHOD_TEMPLATES;
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    ptrTemplateCfg = &cy_capsense_smTemplates[0u];
    ...
    for (i = 0u; i < context->ptrCommonConfig->numWd; i++)
    {
        ...
        if (ptrMapping[snsMethodInternal] == CY_CAPSENSE_REG_MODE_UNDEFINED)
        {
            ...
            else
            {
                ...
                ptrBaseCfg->mode[idCounter] = *ptrTemplateCfg[swTemplateIndex].mode;
    ...
    if (0u != context->ptrCommonConfig->csxEn)
    {
        ...
        ptrBaseCfg->swSelCswFunc[idCounter] = ptrTemplateCfg[1u].swSelCsw[0u];
        idCounter++;
        /* CY_CAPSENSE_CTRLMUX_STATE_TX */
        ptrMapping[CY_CAPSENSE_CTRLMUX_STATE_TX] = idCounter;
        ptrBaseCfg->swSelCswFunc[idCounter] = ptrTemplateCfg[1u].swSelCsw[1u];
        idCounter++;
        /* CY_CAPSENSE_CTRLMUX_STATE_GND */
        ptrMapping[CY_CAPSENSE_CTRLMUX_STATE_GND] = idCounter;
        ptrBaseCfg->swSelCswFunc[idCounter] = ptrTemplateCfg[1u].swSelCsw[2u];
        idCounter++;
        ...
        if (CY_CAPSENSE_ENABLE == context->ptrCommonConfig->mptxEn)
        {
            ...
            ptrBaseCfg->swSelCswFunc[idCounter] = ptrTemplateCfg[1u].swSelCsw[3u];
            idCounter++;
        }
    }
    ...
    if (0u != context->ptrCommonConfig->csdEn)
    {
        ...
        ptrBaseCfg->swSelCswFunc[idCounter] = ptrTemplateCfg[0u].swSelCsw[0u];
        idCounter++;
        /* CY_CAPSENSE_CTRLMUX_STATE_HIGH_Z */
        if (CY_CAPSENSE_SNS_CONNECTION_HIGHZ == context->ptrCommonConfig->csdInactiveSnsConnection)
        {
            ptrMapping[CY_CAPSENSE_CTRLMUX_STATE_HIGH_Z] = idCounter;
            ptrBaseCfg->swSelCswFunc[idCounter] = 0u;
            idCounter++;
        }
        /* CY_CAPSENSE_SNS_CONNECTION_GROUND */
        if (CY_CAPSENSE_SNS_CONNECTION_GROUND == context->ptrCommonConfig->csdInactiveSnsConnection)
        {
                ...
                cy_stc_capsense_context_t * context)
{
    ...
    {
        if (chId == context->ptrPinConfig[i].chId)
        {
    ...
    {
        scanSlotIndexValid = scanSlotIndex + chId * context->ptrCommonConfig->numSlots;
        Cy_CapSense_GenerateSensorConfig(chId, scanSlotIndexValid, &sensorCfgTmp[0], context);
        ...
        if (CY_CAPSENSE_CTRLMUX_SENSOR_CONNECTION_METHOD == context->ptrCommonConfig->sensorConnection)
        {
            ...
            else
            {
                ...
                if (CY_CAPSENSE_CSX_GROUP == widgetSenseGroup)
                {
                    ...
                    if (CY_CAPSENSE_SLOT_SHIELD_ONLY != slotValue)
                    {
                            ...
                            (context->ptrWdConfig[wdIndex].mptxOrder >= CY_CAPSENSE_MPTX_MIN_ORDER))
                        {
                            ...
                            if (0u != i)
                            {
                                pattern = (pattern << i) | (pattern >> (CY_CAPSENSE_MPTX_MAX_ORDER - i));
                            }
                ...
                if (CY_CAPSENSE_SHIELD_DISABLED != context->ptrCommonConfig->csdShieldMode)
                {
                    ...
                    {
                        if (chId == context->ptrShieldPinConfig[i].chId)
                        {
